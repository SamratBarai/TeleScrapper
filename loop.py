...
def downloadVIDs(urlCounter, postFolder, postId, lastVIDPost, mainPath, videoUrls):
    ...
    for url in videoUrls:
        ...
        # Finish messege
        print("[INFO] Downloaded ", vidCounter, " image as ", file_path + str(vidCounter) + ".mp4")
        vidCounter += 1
...
if __name__ == "__main__":
    ...
    # Inputs
    startID = int(input("[SYSTEM][INPUT] => Enter start id for urls: "))
    count = int(input("[SYSTEM][INPUT] => Enter count of ids: "))
    skip = int(input("[SYSTEM][INPUT] => Enter the skip steps: "))
    ...
    while True:
        links.append(f"https://t.me/{username}/{currID}?single")
        currID += skip
        if currID > (startID + count * skip):
            break
